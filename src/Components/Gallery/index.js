...
class Index extends Component {
    ...
    async componentDidMount(){
        switch(this.props.id){
            case 'Samsung':{
            await fetch('http://mohab25.pythonanywhere.com/store/Samsung_Gallery/').then(res=>res.json()).then(
            (data)=>{this.setState({gallery_items:data})})
            };break; 
            case 'Apple':{           
            await fetch('http://mohab25.pythonanywhere.com/store/Apple_Gallery/').then(res=>res.json()).then(
            (data)=>{this.setState({gallery_items:data})})};break; 
            case 'Huawei':{           
                await fetch('http://mohab25.pythonanywhere.com/store/Huawei_Gallery/').then(res=>res.json()).then(
                (data)=>{this.setState({gallery_items:data})})};break; 
            default:{
                await fetch('http://mohab25.pythonanywhere.com/components/GalleryItems/').then(res=>res.json()).then(
                    (data)=>{this.setState({gallery_items:data})}
                )
            }    
    ...
    render() { 
        ...
        if(this.state.gallery_items_sub.length===0){
            ...
            return (
                    <div ref={this.props.GalleryRef} id={this.props.id} style={this.props.id=='Apple'?{marginTop:'-85px'}:{}}>
                        <div className='Gallery-container'>
                            <div className='Gallery-title-container'>
                                <h1 className='Gallery-title'>{this.iconfinder()} Brand Products</h1>
                                <h2 className='Gallery-subtitle'>Check out the our collection of {this.props.id} Best pieces</h2>
                            </div>
                            <div id='Gallery-grid'>
...
export default React.forwardRef((props,ref)=>(<Index {...props} GalleryRef={ref}/>))